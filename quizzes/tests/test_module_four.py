"""Module 4 (Secure Communication Practices) meets the Module 1 gold standard.

Mirrors the content contracts against order_index=4: every panel is deep reading
...
`harden` (secure-a-workspace) contract, and a full interactive journey proving
...
# The interactive "do it" kinds Module 4 draws on (the new harden included).
ACTIVITY_KINDS = {"SORT", "INBOX", "SPOT", "PASSWORD", "BRANCH", "CLASSIFY", "MAILSORT", "HARDEN"}
...
# --------------------------------------------------------------------------
# Topic + activities: true to "Secure Communication Practices"
# --------------------------------------------------------------------------


@pytest.mark.django_db
def test_uses_its_planned_activity_set_including_the_new_harden(seeded):
    kinds = set(
        LessonTask.objects.filter(lesson__module=seeded).values_list("kind", flat=True)
    )
    assert {"SORT", "PASSWORD", "CLASSIFY", "HARDEN"} <= kinds, f"missing: {kinds}"


@pytest.mark.django_db
def test_uses_the_encryption_diagram(seeded):
    keys = set(
...
@pytest.mark.django_db
def test_harden_activity_is_well_formed_and_solvable(seeded):
    ...
    for task in tasks:
        ...
        steps = p["steps"]
        assert len(steps) >= 4, f"{task.task_key} needs several steps"
        for s in steps:
            assert s.get("label") and s.get("risk"), f"{task.task_key}/{s['id']} needs a label and risk"
            assert len(s["options"]) >= 2, f"{task.task_key}/{s['id']} needs choices"
            assert sum(1 for o in s["options"] if o["correct"]) == 1, \
                f"{task.task_key}/{s['id']} needs exactly one secure option"
            for o in s["options"]:
                assert o.get("text") and o.get("why"), f"{task.task_key}/{s['id']} option needs text and why"
...
def test_every_question_is_well_formed_and_traces_to_a_lesson(seeded):
    ...
    for q in seeded.quiz.questions.all():
        ...
        assert q.lesson_reference_id in lesson_ids, f"{q} points outside Module 4"


# --------------------------------------------------------------------------
# The full interactive journey: learn -> quiz -> pass -> Module 5 unlocks
# --------------------------------------------------------------------------
...
def learner_at_module_four(client, db):
    ...
    # Unlock Module 4 by completing Modules 1 to 3 (lessons + passed quizzes).
    for idx in (1, 2, 3):
...
def test_full_interactive_module_four_journey(learner_at_module_four):
    ...
    assert "Module 04 complete" in overview
    assert "Start Module 5" in overview
    assert reverse("learn:module", args=[5]) in overview
